<div class="box">
	<div class="box-header">
		<ul class="nav nav-tabs">
			...
			<li role="presentation" class="nav-item"><a class="nav-link active" href="<?php echo admin_url($this->pagedir . '/changepw'); ?>" onclick="return check_form_changed();">패스워드변경</a></li>
	...
	<div class="box-table">
		...
		?>
			...
			<div class="alert alert-success">
				...
				<p><strong>{회원아이피}</strong> 는 패스워드변경시 접속한 실제 IP 로 치환됩니다</p>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<div class="form-group col-sm-12">
						<input type="text" class="form-control" name="send_email_changepw_admin_title" id="send_email_changepw_admin_title" value="<?php echo set_value('send_email_changepw_admin_title', element('send_email_changepw_admin_title', element('data', $view))); ?>" />
					</div>
					<div class="form-group col-sm-12">
						<?php echo display_dhtml_editor('send_email_changepw_admin_content', set_value('send_email_changepw_admin_content', element('send_email_changepw_admin_content', element('data', $view))), $classname = 'form-control dhtmleditor', $is_dhtml_editor = true, $editor_type = "smarteditor"); ?>
					</div>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<div class="form-group col-sm-12">
						<input type="text" class="form-control" name="send_email_changepw_user_title" id="send_email_changepw_user_title" value="<?php echo set_value('send_email_changepw_user_title', element('send_email_changepw_user_title', element('data', $view))); ?>" />
					</div>
					<div class="form-group col-sm-12">
						<?php echo display_dhtml_editor('send_email_changepw_user_content', set_value('send_email_changepw_user_content', element('send_email_changepw_user_content', element('data', $view))), $classname = 'form-control dhtmleditor', $is_dhtml_editor = true, $editor_type = "smarteditor"); ?>
					</div>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<div class="form-group col-sm-12">
						<input type="text" class="form-control" name="send_note_changepw_admin_title" id="send_note_changepw_admin_title" value="<?php echo set_value('send_note_changepw_admin_title', element('send_note_changepw_admin_title', element('data', $view))); ?>" />
					</div>
					<div class="form-group col-sm-12">
						<?php echo display_dhtml_editor('send_note_changepw_admin_content', set_value('send_note_changepw_admin_content', element('send_note_changepw_admin_content', element('data', $view))), $classname = 'form-control dhtmleditor', $is_dhtml_editor = true, $editor_type = "smarteditor"); ?>
					</div>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<div class="form-group col-sm-12">
						<input type="text" class="form-control" name="send_note_changepw_user_title" id="send_note_changepw_user_title" value="<?php echo set_value('send_note_changepw_user_title', element('send_note_changepw_user_title', element('data', $view))); ?>" />
					</div>
					<div class="form-group col-sm-12">
						<?php echo display_dhtml_editor('send_note_changepw_user_content', set_value('send_note_changepw_user_content', element('send_note_changepw_user_content', element('data', $view))), $classname = 'form-control dhtmleditor', $is_dhtml_editor = true, $editor_type = "smarteditor"); ?>
					</div>
				</div>
			</div>
			<div class="form-group">
...
$(function() {
	$('#fadminwrite').validate({
		rules: {
			send_email_changepw_admin_content : {'required_smarteditor' : true },
			send_email_changepw_user_content : {'required_smarteditor' : true },
			send_note_changepw_admin_content : {'required_smarteditor' : true },
			send_note_changepw_user_content : {'required_smarteditor' : true }
		}
	});
});
...
$(document).on('click', '.reset_email_to_user', function() {
	...
	oEditors.getById["send_email_changepw_user_content"].exec("SET_CONTENTS", [sHTML]);
});
$(document).on('click', '.reset_note_to_admin', function() {
	$('#send_note_changepw_admin_title').val('{회원닉네임}님이 패스워드를 변경하셨습니다');
	var sHTML = '<table width="100%" border="0" cellpadding="0" cellspacing="0" style="border-left: 1px solid rgb(226,226,225);border-right: 1px solid rgb(226,226,225);background-color: rgb(255,255,255);border-top:10px solid #000000; border-bottom:5px solid #000000;border-collapse: collapse;"><tr><td style="font-size:12px;padding:20px 30px;font-family: Arial,sans-serif;color: rgb(0,0,0);font-size: 14px;line-height: 20px;"><span style="font-size:14px;font-weight:bold;color:rgb(0,0,0)">안녕하세요 관리자님,</span><br /></td></tr><tr style="border-top:1px solid #e2e2e2; border-bottom:1px solid #e2e2e2;"><td style="padding:20px 30px;font-family: Arial,sans-serif;color: rgb(0,0,0);font-size: 14px;line-height: 20px;"><p>안녕하세요,</p><p>{회원닉네임} 님이 패스워드를 변경하셨습니다.</p><p>회원아이디 : {회원아이디}</p><p>닉네임 : {회원닉네임}</p><p>이메일 : {회원이메일}</p><p>변경한 곳 IP : {회원아이피}</p><p>감사합니다.</p></td></tr><tr><td style="padding:10px;font-family: Arial,sans-serif;color: rgb(0,0,0);font-size: 14px;line-height: 20px;text-align:center;">{홈페이지명}</td></tr></table>';
	oEditors.getById["send_note_changepw_admin_content"].exec("SET_CONTENTS", [sHTML]);
});
