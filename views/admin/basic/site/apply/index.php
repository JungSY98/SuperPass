...
<div class="box">
  ...
  <ul class="nav nav-tabs mb30" role="tablist">
    <li role="presentation"<?=$viewCate=='기계금속' ? ' class="active"' : ''?>><a href="/admin/site/apply/?viewCate=기계금속&status=<?=$status?>" role="tab" style="font-size:20px;" >기계금속</a></li>
    <li role="presentation"<?=$viewCate=='인쇄' ? ' class="active"' : ''?>><a href="/admin/site/apply/?viewCate=인쇄&status=<?=$status?>" role="tab" style="font-size:20px;" >인쇄</a></li>
    <li role="presentation"<?=$viewCate=='주얼리' ? ' class="active"' : ''?>><a href="/admin/site/apply/?viewCate=주얼리&status=<?=$status?>" role="tab" style="font-size:20px;">주얼리</a></li>
	<li role="presentation"<?=$viewCate=='수제화' ? ' class="active"' : ''?>><a href="/admin/site/apply/?viewCate=수제화&status=<?=$status?>" role="tab" style="font-size:20px;">수제화</a></li>

  </ul>

	<div class="input-group-btn mb30">
	<a href="/admin/site/apply/?viewCate=<?=$viewCate?>&status=접수" role="tab" class="btn btn-default" >접수 리스트 보기</a>
	<a href="/admin/site/apply/?viewCate=<?=$viewCate?>&status=적격" role="tab" class="btn btn-success">적격 리스트 보기</a>
	<a href="/admin/site/apply/?viewCate=<?=$viewCate?>&status=부적격" role="tab" class="btn btn-danger">부적격 리스트 보기</a>
	<a href="/admin/site/apply/dnAttachFile/?viewCate=<?=$viewCate?>" class="btn btn-default pull-right"><?=$viewCate?> 첨부파일 일괄다운</a>
	</div>
...
<? } ?>
		...
		<div class="input-group-btn mb30">
			<a href="/admin/site/apply/dnList/?year=<?=$year?>" class="btn btn-success">전체 리스트 다운</a>
			<a href="/admin/site/apply/dnList/?cate=기계금속&status=<?=$status?>&year=<?=$year?>" class="btn btn-info">기계금속 리스트 다운 (<?=element('기계금속', $cntD)?>)</a>
			<a href="/admin/site/apply/dnList/?cate=인쇄&status=<?=$status?>&year=<?=$year?>" class="btn btn-info">인쇄 리스트 다운 (<?=element('인쇄', $cntD)?>)</a>
			<a href="/admin/site/apply/dnList/?cate=주얼리&status=<?=$status?>&year=<?=$year?>" class="btn btn-info">주얼리 리스트 다운 (<?=element('주얼리', $cntD)?>)</a>
			<a href="/admin/site/apply/dnList/?cate=수제화&status=<?=$status?>&year=<?=$year?>" class="btn btn-info">수제화 리스트 다운 (<?=element('수제화', $cntD)?>)</a>
		</div>
...
function fnViewFile(no) {
	...
	$.ajax({
		method: "POST",
		url: "/admin/site/apply/ajaxGetDocument/",
		data: {
			'no' : no,
...
function fnSetEval(act) {
	...
	if (viewNo != '' && confirm(msg)) {
		$.ajax({
			method: "POST",
			url: "/admin/site/apply/ajaxSetStatus/",
			data: {
				'no' : viewNo,
				'act' : act,
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
...
function fnChgCategory(no, cate) {
	if (confirm(cate+" 분류로 변경하시겠습니까?")) {
		$.ajax({
			method: "POST",
			url: "/admin/site/apply/ajaxChgCategory/",
			data: {
				'no' : no,
				'cate' : cate,
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
