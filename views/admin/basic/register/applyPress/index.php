<div class="row">
	<div class="col-sm-12">
		...
		<div class="input-group mb-4">
			<a href="/admin/register/applyPress/dnList/" class="btn btn-success">전체 리스트 다운</a>
		</div>
...
<? foreach($applyD as $k => $aD) { ?>
			<tr>
				...
				<td><?=$aD['comName']?> / <?=$aD['comAddr']?> / <?=$aD['comContract']?></td>
				...
				<td class="text-center">
					...
					<a href="/admin/register/applyPress/form/<?=$aD['no']?>/" class="btn btn-secondary">상세</a>
...
function fnSetStatus2(no, value) {
	if (value!="" && confirm(value+" 상태로 변경하시겠습니까?")) {
		$.ajax({
			url: "/admin/register/applyPress/ajaxSetStatus/",
			type: "POST",
			dataType: "html",
			data: {
				'value': value,
