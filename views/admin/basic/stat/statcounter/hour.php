	...
	<div class="box-table">
		<div class="box-table-header">
			<ul class="nav nav-pills">
				<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir); ?>">방문자로그 </a></li>
				<li role="presentation" class="nav-item"><a class="nav-link active" href="<?php echo admin_url($this->pagedir . '/visit'); ?>">기간별 그래프</a></li>
				<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/cleanlog'); ?>">오래된 로그삭제</a></li>
			</ul>
			<form class="form-inline" name="flist" method="get" >
				<div class="box-table-button">
					...
					<div class="btn-group mt-4 mb-4" role="group" aria-label="...">
						<button data-page-url="<?php echo admin_url($this->pagedir . '/visit'); ?>" class="btn btn-secondary btn-sm statsubmit">방문자</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/domain'); ?>" class="btn btn-secondary btn-sm statsubmit">도메인</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/browser'); ?>" class="btn btn-secondary btn-sm statsubmit">브라우저</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/os'); ?>" class="btn btn-secondary btn-sm statsubmit">운영체제</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/hour'); ?>" class="btn btn-warning btn-sm statsubmit">시간</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/week'); ?>" class="btn btn-secondary btn-sm statsubmit">요일</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/day'); ?>" class="btn btn-secondary btn-sm statsubmit">일</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/month'); ?>" class="btn btn-secondary btn-sm statsubmit">월</button>
						<button data-page-url="<?php echo admin_url($this->pagedir . '/year'); ?>" class="btn btn-secondary btn-sm statsubmit">년</button>
					</div>
				</div>
			</form>
		...
		<div class="table-responsive">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				?>
					<tr class="<?php echo ( ! element('cnt', $result)) ? 'zerodata' : ''; ?>">
						<td><?php echo $key; ?>:00</td>
						<td><?php echo number_format(element('cnt', $result, 0)); ?></td>
...
function drawChart() {
	...
	data.addRows([
		...
		if (element('list', $view)) {
			foreach (element('list', $view) as $key => $result) {
		?>
		['<?php echo $key; ?>시',<?php echo element('cnt', $result, 0); ?>],
		<?php
			}
		}
		?>
...
$(document).on('click', '#export_to_excel', function(){
	exporturl = '<?php echo admin_url($this->pagedir . '/hour/excel' . '?' . $this->input->server('QUERY_STRING', null, '')); ?>';
