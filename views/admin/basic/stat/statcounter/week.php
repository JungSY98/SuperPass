...
function drawChart() {
	...
	data.addRows([
		...
		if (element('list', $view)) {
			foreach (element('list', $view) as $key => $result) {
		?>
		['<?php echo element($key, element('week_korean', $view)); ?>',<?php echo element('scd_count', $result, 0); ?>],
		<?php
			}
		}
		?>
...
$(document).on('click', '#export_to_excel', function(){
	exporturl = '<?php echo admin_url($this->pagedir . '/week/excel' . '?' . $this->input->server('QUERY_STRING', null, '')); ?>';
