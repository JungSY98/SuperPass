	...
	<div class="box-header">
		<div class="box-table-header">
			<ul class="nav nav-pills">
				<li role="presentation" class="nav-item"><a href="<?php echo admin_url($this->pagedir); ?>" class="nav-link">배너클릭</a></li>
				<li role="presentation" class="nav-item"><a href="<?php echo admin_url($this->pagedir . '/graph'); ?>" class="nav-link active ">기간별 그래프</a></li>
			...
			<form class="form-inline" name="flist" action="<?php echo current_url(); ?>" method="get" >
				...
				<div class="box-table-button p-5">
					<div class="input-group">
						<span class="input-group-text">기간 : </span>
						<input type="text" class="form-control input-small datepicker " name="start_date" value="<?php echo element('start_date', $view); ?>" readonly="readonly" />
						<span class="input-group-text"> - </span>
						<input type="text" class="form-control input-small datepicker" name="end_date" value="<?php echo element('end_date', $view); ?>" readonly="readonly" />
						<button type="button" class="btn <?php echo ($this->input->get('datetype') !== 'y' && $this->input->get('datetype') !== 'm') ? 'btn-success' : 'btn-light'; ?>" onclick="fdate_submit('d');">일별보기</button>
						<button type="button" class="btn <?php echo ($this->input->get('datetype') === 'm') ? 'btn-success' : 'btn-light'; ?>" onclick="fdate_submit('m');">월별보기</button>
						<button type="button" class="btn <?php echo ($this->input->get('datetype') === 'y') ? 'btn-success' : 'btn-light'; ?>" onclick="fdate_submit('y');">년별보기</button>
					</div>
				</div>
			</form>
