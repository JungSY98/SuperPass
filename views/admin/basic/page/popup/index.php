		...
		?>
			<div class="box-table-header w-100">
				<div class="btn-group" role="group">
					<a href="?" class="btn <?php echo ($this->input->get('pop_activated') !== 'Y' && $this->input->get('pop_activated') !== 'N') ? 'btn-success' : 'btn-secondary'; ?>">전체배너</a>
					<a href="?pop_activated=Y" class="btn <?php echo ($this->input->get('pop_activated') === 'Y') ? 'btn-success' : 'btn-secondary'; ?>">활성</a>
					<a href="?pop_activated=N" class="btn <?php echo ($this->input->get('pop_activated') === 'N') ? 'btn-success' : 'btn-secondary'; ?>">비활성</a>
				</div>
				<?php
				ob_start();
				?>
					<div class="btn-group float-end" role="group" aria-label="...">
						...
						<button type="button" class="btn btn-outline btn-secondary btn-sm btn-list-delete btn-list-selected disabled" data-list-delete-url = "<?php echo element('list_delete_url', $view); ?>" >선택삭제</button>
				...
				<table class="table table-hover table-striped">
					...
					?>
						<tr>
							...
							<td><a href="<?php echo admin_url($this->pagedir); ?>/write/<?php echo element(element('primary_key', $view), $result); ?>?<?php echo $this->input->server('QUERY_STRING', null, ''); ?>" class="btn btn-outline btn-secondary btn-xs">수정</a></td>
