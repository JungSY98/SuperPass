<div class="box">
	<div class="box-header">
		<ul class="nav nav-tabs">
			<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir); ?>">기본정보</a></li>
			<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/cleantodormant'); ?>">휴면계정일괄정리</a></li>
			<li role="presentation" class="nav-item"><a class="nav-link active" href="<?php echo admin_url($this->pagedir . '/emailtowaiting'); ?>">안내메일일괄발송</a></li>
			<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/emailcontent'); ?>">안내메일내용</a></li>
			<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/emailsendlist'); ?>">안내메일발송내역</a></li>
			<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/waitinglist'); ?>">휴면처리해야할회원</a></li>
			<li role="presentation" class="nav-item"><a class="nav-link" href="<?php echo admin_url($this->pagedir . '/dormantlist'); ?>">휴면중인회원</a></li>
		</ul>
	</div>
	<div class="box-table">
...
$(document).on('click', '.execute_clean', function() {
	if (confirm('정말 메일발송을 시작하시겠습니까?')) {
		...
		$.ajax({
			...
			data : {execute : '1', csrf_entropy_form : cb_csrf_hash},
			async: true,
			method : 'post',
			cache: false,
			dataType: 'json',
