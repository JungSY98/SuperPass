		...
		?>
			<div class="box-table-header w-100">
				<div class="btn-group" role="group">
					<a href="?" class="btn <?php echo ( ! $this->input->get('mem_is_admin') && ! $this->input->get('mem_denied')) ? 'btn-success' : 'btn-light'; ?>">전체회원</a>
					<a href="?mem_is_admin=1" class="btn <?php echo ($this->input->get('mem_is_admin')) ? 'btn-success' : 'btn-light'; ?>">최고관리자</a>
					<a href="?mem_denied=1" class="btn <?php echo ($this->input->get('mem_denied')) ? 'btn-success' : 'btn-light'; ?>">차단회원</a>
				</div>
				<div class="btn-group" role="group">
					<a href="?" class="btn <?php echo ( ! $this->input->get('mgr_id')) ? 'btn-success' : 'btn-light'; ?>">전체그룹</a>
					<?php
					foreach (element('all_group', $view) as $gkey => $gval) {
					?>
						<a href="?mgr_id=<?php echo element('mgr_id', $gval); ?>" class="btn <?php echo (element('mgr_id', $gval) === $this->input->get('mgr_id')) ? 'btn-success' : 'btn-light'; ?>"><?php echo element('mgr_title', $gval); ?></a>
					<?php
					}
					?>
				</div>
				...
				?>
					<div class="btn-group float-end" role="group" aria-label="...">
						<button type="button" class="btn btn-outline btn-success" id="export_to_excel"><i class="fa fa-file-excel-o"></i> 엑셀 다운로드</button>
						...
						<a href="<?php echo element('write_url', $view); ?>" class="btn btn-danger">회원추가</a>
					</div>
				<?php
				...
				<table class="table table-hover table-striped">
					<thead>
						<tr>
							...
							<th><a href="<?php echo element('mem_userid', element('sort', $view)); ?>">아이디</a><br><a href="<?php echo element('mem_email', element('sort', $view)); ?>">이메일</a></th>
					...
					?>
						<tr>
							...
							<td>
								<?php echo html_escape(element('mem_userid', $result)); ?><br>
								<?php echo html_escape(element('mem_email', $result)); ?>
							</td>
							<td>
				...
				<div class="col-md-6 offset-md-3">
					<div class="input-group">
						...
						<span class="input-group-btn">
							<button class="btn btn-light btn-sm" name="search_submit" type="submit">검색!</button>
						</span>
					</div>
				</div>
			</div>
