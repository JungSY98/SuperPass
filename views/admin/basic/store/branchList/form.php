...
<div class="card">
	...
	<div class="card-body">
<?=form_open_multipart("/admin/store/branchList/save", array("onsubmit"=>"return fnChkForm()"), array("sNo"=>$sNo))?>
		<table class="table" id="spotTable">
			...
			<tr>
				<th>스탬프여부</th>
				<td>
					<label><input type="radio" name="isStamp"  value="Y" <?=(element("isStamp", $branchD) == 'Y' || empty($bNo)) ? 'checked' : ''?>> 예</label>
					<label><input type="radio" name="isStamp"  value="N" <?=(element("isStamp", $branchD) == 'N' || empty($bNo)) ? 'checked' : ''?>> 아니오</label>
				</td>
			</tr>
			<tr>
				<th>대분류 Store 명</th>
				<td>
					<div class="input-group">
						...
						<input type="text" name="sNameKR" class="form-control" placeholder="국문 Store명을 입력하여 주세요" required value="<?=element("sNameKR", $branchD)?>">
					</div>
					<div class="input-group">
						<span class="input-group-text">영문</span>
						<input type="text" name="sNameEN" class="form-control" placeholder="영문 Store명을입력하여 주세요"  value="<?=element("sNameEN", $branchD)?>">
					</div>
				</td>
			</tr>
			<tr>
				<th class="col-sm-2">대분류 Store 소개 (국문)</th>
				<td><textarea name="sDescKR" class="form-control" placeholder="Store 소개 (국문)를 입력하여 주세요" rows=7><?=element("sDescKR", $branchD)?></textarea></td>
			</tr>

			<tr>
				<th class="col-sm-2">대분류 Store 소개 (영문)</th>
				<td><textarea name="sDescEN" class="form-control" placeholder="Store 소개 (국문)를 입력하여 주세요" rows=7 ><?=element("sDescEN", $branchD)?></textarea></td>
			</tr>
			<tr class="">
				<th>대분류 Store 메인 이미지</th>
				<td>
...
<? if (element("sMainIMG_Source", $branchD)) { ?>
					<img src="/uploads/branch/<?=element("sMainIMG_Source", $branchD)?>" style="width:200px;">
					<label><input type="checkbox" name="del_spotIMG" value="<?=element("sMainIMG_Source", $branchD)?>"> 이미지 삭제</label>
					<input type="hidden" name="sMainIMG_Name" value="<?=element("sMainIMG_Name", $branchD)?>">
					<input type="hidden" name="sMainIMG_Source" value="<?=element("sMainIMG_Source", $branchD)?>">
<? } ?>
...
<? } ?>
			...
			<tr>
				...
				<td>
					<div class="input-group w-100">
						<input type="text" id="sAddr1KR" name="sAddr1KR" class="form-control" placeholder="국문 주소를 입력하여 주세요"  value="<?=element("sAddr1KR", $branchD)?>" readonly>
						<button class="btn btn-success" type="button" onclick="fnSearchAddr()">주소 검색</button>
					</div>
					<input type="text" id="sAddr2KR" name="sAddr2KR" class="form-control" placeholder="국문 상세주소를 입력하여 주세요"  value="<?=element("sAddr2KR", $branchD)?>">
					<input type="hidden" id="sAddr3KR" name="sAddr3KR" class="form-control" value="<?=element("sAddr3KR", $branchD)?>">
				</td>
			</tr>
			<tr>
				<th class="col-sm-2">주소 (영문)</th>
				<td>
					<input type="text" id="sAddr1EN" name="sAddr1EN" class="form-control" placeholder="영문 주소를 입력하여 주세요"  value="<?=element("sAddr1EN", $branchD)?>" readonly>
					<input type="text" id="sAddr2EN" name="sAddr2EN" class="form-control" placeholder="영문 상세주소를 입력하여 주세요"  value="<?=element("sAddr2EN", $branchD)?>">
					<input type="hidden" id="sAddr3EN" name="sAddr3EN" class="form-control" value="<?=element("sAddr3EN", $branchD)?>">
				</td>
			</tr>
			<tr>
				<th class="col-sm-2">주소 (영문)</th>
				<td>
					...
					<div class="input-group">
						...
						<input type="text" class="form-control" name="mapLat" id="mapLat" value="<?=element("mapLat", $branchD)?>">
						<span class="input-group-text">Lng</span>
						<input type="text" class="form-control" name="mapLng"id="mapLng"  value="<?=element("mapLng", $branchD)?>">
					</div>
			...
			<tr>
				<th>운영시간</th>
				<td><input type="text" name="sOpenTime" class="form-control" placeholder="운영시간을 입력하여 주세요"  value="<?=element("sOpenTime", $branchD)?>"></td>
			</tr>
			<tr>
				<th>연락처</th>
				<td><input type="text" name="sContact" class="form-control" placeholder="연락처를 입력하여 주세요"  value="<?=element("sContact", $branchD)?>"></td>
			</tr>
			<tr>
				<th>홈페이지 / SNS</th>
				<td>
					<div class="input-group mb10">
						...
						<input type="text" name="sLink1" class="form-control" placeholder="홈페이지를 입력하여 주세요"  value="<?=element("sLink1", $branchD)?>">
					</div>
					<div class="input-group mb10">
						<span class="input-group-text">인스타</span>
						<span class="input-group-text">https://www.instagram.com/</span>
						<input type="text" name="sLink2" class="form-control" placeholder="인스타를 입력하여 주세요"  value="<?=element("sLink2", $branchD)?>">
					</div>
					<div class="input-group">
						<span class="input-group-text">기타</span>
						<textarea name="sLink3" class="form-control" placeholder="기타 SNS를 입력하여 주세요"  rows="4"><?=element("sLink3", $branchD)?></textarea>
					</div>
				</td>
			</tr>
			<tr id="attachTR">
...
<?
	if (is_array(element('sAddFieldKR', $branchD)))
		foreach($branchD['sAddFieldKR'] as $k4 => $fD)  {
			$krField = $fD;
			$enField = element($k4, element('sAddFieldEN', $branchD));
?>
			<tr class="addTR">
				<td>
					<div class="input-group">
			...
			<tr>
				...
				<td>
					<div class="row">
						...
						<div class="col-6">
							<button type="button" class="btn btn-secondary w-100" onclick="location = '/admin/store/branchList/'; ">목록으로</button>
						</div>
					</div>
				</td>
			</tr>
...
function fnGetImgExplain() {
	$.ajax({
		url: "/admin/store/branchList/ajaxGetUploadPhoto/",
		type: "POST",
		dataType: "html",
		data: {
			'type': 'branchExplain',
...
function fnCallUpload(target, expTarget) {
	...
	$("#"+target).pluploadQueue({
		// General settings
		runtimes : 'html5',
		url : "/admin/store/branchList/upload",
		chunk_size : '1mb',
		unique_names : true,
		multipart_params: { 
			'spotNo' : '<?=$sNo?>',
			...
			UploadComplete: function(up, files) {
				...
				for( var i = 0 ; i < fileVerify.length ; i++) {
					...
					if (fileVerify[i].target_name.indexOf(".jpg") > 0 || fileVerify[i].target_name.indexOf(".jpeg") > 0 || fileVerify[i].target_name.indexOf(".png") > 0 || fileVerify[i].target_name.indexOf(".gif") > 0) {
						upVerifyHtml += "<img class='mb-1' src='/uploads/branch/"+fileVerify[i]['target_name']+"'>";
					} else {
						upVerifyHtml += "<a href='/files/"+fileVerify[i]['name']+"/branch/file/"+fileVerify[i]['target_name']+"/' class='btn btn-dark w-100'>"+fileVerify[i]['target_name']+"</a>";
					}
...
function fnUploadSave() {
	...
	if ($(".uploadFileNames").length>0) {
		...
		$.ajax({
			url: "/admin/store/branchList/ajaxUploadPhoto/",
			type: "POST",
			dataType: "html",
			data: {
				'type': 'branchExplain',
...
function fnUpdatePhotoInfoFromModal() {
	...
	$.ajax({
		url: "/admin/store/branchList/ajaxUpdatePhotoInfo/",
		type: "POST",
		data: {
			'image_no': image_no,
...
function fnDelFile(no) {
	if (confirm("정말 삭제하시겠습니까?\n\n한번 삭제한 자료는 복구할 수 없습니다.")) {
		$.ajax({
			url: "/admin/store/branchList/ajaxDelUploadPhoto/",
			type: "POST",
			dataType: "html",
			data: {
				'no': no,
