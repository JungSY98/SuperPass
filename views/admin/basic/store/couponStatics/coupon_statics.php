...
<form id="frmFilter" class="row gy-2 gx-3 align-items-end mb-3">
	<div class="col-auto">
		...
		<div class="d-flex gap-2">
			<input type="text" class="form-control form-control-sm datepicker" name="start" value="<?=html_escape($start)?>" placeholder="YYYY-MM-DD" style="width:120px;">
			<input type="text" class="form-control form-control-sm datepicker" name="end"	 value="<?=html_escape($end)?>"	 placeholder="YYYY-MM-DD" style="width:120px;">
		</div>
	</div>
	<div class="col-auto">
...
<div class="tab-content">
	...
	<div class="tab-pane fade show active" id="tab1">
		<!-- KPI -->
		<div class="row g-3 kpi my-1">
			<div class="col-lg-3 col-md-6"><div class="card p-3"><div class="lbl">총 사용수</div><div class="val" id="kpiUsage">0</div></div></div>
			<div class="col-lg-3 col-md-6"><div class="card p-3"><div class="lbl">고유회원수</div><div class="val" id="kpiMember">0</div></div></div>
			<div class="col-lg-3 col-md-6"><div class="card p-3"><div class="lbl">총 할인액</div><div class="val" id="kpiDiscount">0</div></div></div>
			<div class="col-lg-3 col-md-6"><div class="card p-3"><div class="lbl">총 결제액</div><div class="val" id="kpiPay">0</div></div></div>
		</div>
		<!-- ADD: 파생 KPI -->
...
// Export
$('#btnCSV').on('click',	()=> location.href='/admin/store/couponStatics/export/csv?'	+ q());
$('#btnXLSX').on('click', ()=> location.href='/admin/store/couponStatics/export/xlsx?' + q());

/* ===== Ajax Loaders ===== */
function loadSummary(){
	const qStr = q();
	// 전체 요약
	$.getJSON('/admin/store/couponStatics/summary', qStr)
		.done(r=>{
			...
			$.getJSON('/admin/store/couponStatics/summary', qStamp).done(rs=>{
...
function loadTopStore(){
	$.getJSON('/admin/store/couponStatics/top-stores', q())
		.done(r=>{
...
function loadDevice(){
	$.getJSON('/admin/store/couponStatics/device', q())
		.done(r=>{
			const labels=[], values=[];
			(r.rows||[]).forEach(x=>{
...
function loadTopCoupon(){
	$.getJSON('/admin/store/couponStatics/top-coupons', q())
		.done(r=>{
...
function loadHeat(){	$.getJSON('/admin/store/couponStatics/heatmap', q()).done(r=> buildHeat($('#tblHeat'),	r.rows||[])).fail(()=>showErr('히트맵 조회 실패')); }
function loadHeat2(){ $.getJSON('/admin/store/couponStatics/heatmap', q()).done(r=> buildHeat($('#tblHeat2'), r.rows||[])).fail(()=>showErr('시간대 표 조회 실패')); }

// 기획문서1: 그룹+소계+합계
function loadCouponGrouped(){
	$.getJSON('/admin/store/couponStatics/coupon-table', q())
		.done(r=>{
			const rows = r.rows||[];
...
// 브랜드/몰
function loadBrand(){
	$.getJSON('/admin/store/couponStatics/group/brand', q())
		.done(r=>{
...
function loadMall(){
	$.getJSON('/admin/store/couponStatics/group/mall', q())
		.done(r=>{
