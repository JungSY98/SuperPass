...
<div class="col-sm-6 mt-5 mb-5 pt-5 pb-5">
	<div class="card panel-default">
		<div class="card-body">
			<h5 class="card-title"><?=$siteTitle['btnFindID']?></h5>
		</div>

		<div class="card-body">
			<?php
			echo validation_errors('<div class="alert alert-warning" role="alert">', '</div>');
			echo show_alert_message(element('message', $view), '<div class="alert alert-auto-close alert-dismissible alert-info"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>', '</div>');
			...
			?>
				...
				<legend><?=$siteTitle['findIDtxt1']?></legend>
				<p><?=$siteTitle['findIDtxt2']?><br><?=$siteTitle['findIDtxt3']?></p>
				<div class="input-group col-md-8">
					<input type="email" name="idpw_email" id="idpw_email" class="form-control" placeholder="Email Address" />
					<span class="input-group-btn">
						<button class="btn btn-dark" type="submit"><?=$siteTitle['btnFindID']?></button>
					</span>
				</div>
			<?php
			echo form_close();
			...
			if ($this->cbconfig->item('use_register_email_auth')) {
				...
				?>
					...
					<div class="input-group col-md-8">
						...
						<span class="input-group-btn">
							<button class="btn btn-dark btn-sm" type="submit">인증메일 재발송</button>
						</span>
					</div>
				<?php
				echo form_close();
				...
				?>
					...
					<div class="form-group">
						<div class="col-md-4 col-lg-offset-3">
							<button type="submit" class="btn btn-dark btn-sm">새로운 이메일주소로 인증메일 재발송</button>
						</div>
					</div>
			<?php
				echo form_close();
