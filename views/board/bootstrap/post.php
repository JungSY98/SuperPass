...
<div class="container-fluid max1920 board">
	...
	<h3>
		...
		<?php echo html_escape(element('title', element('post', $view))); ?>
	</h3>
	<ul class="info mt-4">
		<li><?php echo element('display_name', element('post', $view)); ?></li>
		<li class="copy_post_url" data-clipboard-text="<?php echo element('short_url', $view); ?>"><span><i class="fa fa-link"></i> <?=$siteTitle['boardT1']?></span></li>
		...
		<?php } ?>
		<li class="float-end"><i class="fa fa-clock-o"></i> <?php echo element('display_datetime', element('post', $view)); ?></li>
		<?php if (element('display_ip', element('post', $view))) { ?>
			<li class="float-end"><i class="fa fa-map-marker"></i> <?php echo element('display_ip', element('post', $view)); ?></li>
		<?php } ?>
	</ul>
	<?php if (element('link_count', $view) > 0 OR element('file_download_count', $view) > 0) { ?>
		<ul class="list-group">
			<?php if (element('file_download_count', $view) > 0) { ?>
				<li class="list-group-item list-group-item-success">Download</li>
			...
			if (element('link_count', $view) > 0) { ?>
				<li class="list-group-item list-group-item-success">Link</li>
			<?php
	...
	function file_download(link) {
		<?php if ((int)element('point_filedownload', element('board', $view)) < 0) { ?>if ( ! confirm("파일을 다운로드 하시면 포인트가 차감(<?php echo number_format(element('point_filedownload', element('board', $view))); ?>점)됩니다.\n\n포인트는 게시물당 한번만 차감되며 다음에 다시 다운로드 하셔도 중복하여 차감하지 않습니다.\n\n그래도 다운로드 하시겠습니까?")) { return; }<?php }?>
...
<?php if ( ! element('post_del', element('post', $view)) && element('is_admin', $view)) { ?>
			<button type="button" class="btn btn-dark admin-manage-post"><i class="fa fa-cog big-fa"></i> <?=$siteTitle['boardT5']?></button>
...
<?php if (element('modify_url', $view)) { ?>
			<a href="<?php echo element('modify_url', $view); ?>" class="btn btn-dark"><?=$siteTitle['boardT10']?></a>
<?php } ?>
<?php	if (element('delete_url', $view)) { ?>
			<button type="button" class="btn btn-danger btn-one-delete" data-one-delete-url="<?php echo element('delete_url', $view); ?>"><?=$siteTitle['boardT9']?></button>
<?php } ?>
			<a href="<?php echo element('list_url', $view); ?>" class="btn btn-dark"><?=$siteTitle['boardT2']?></a>
<?php if (element('search_list_url', $view)) { ?>
				<a href="<?php echo element('search_list_url', $view); ?>" class="btn btn-dark"><?=$siteTitle['boardT3']?></a>
<?php } ?>
<?php if (element('reply_url', $view)) { ?>
			<a href="<?php echo element('reply_url', $view); ?>" class="btn btn-dark"><?=$siteTitle['boardT8']?></a>
<?php } ?>
<?php if (element('prev_post', $view)) { ?>
			<a href="<?php echo element('url', element('prev_post', $view)); ?>" class="btn btn-dark"><?=$siteTitle['boardT6']?></a>
<?php } ?>
<?php if (element('next_post', $view)) { ?>
			<a href="<?php echo element('url', element('next_post', $view)); ?>" class="btn btn-dark"><?=$siteTitle['boardT7']?></a>
<?php } ?>
		</div>
<?php if (element('write_url', $view)) { ?>
		<div class="float-end">
			<a href="<?php echo element('write_url', $view); ?>" class="btn btn-success"><?=$siteTitle['boardT4']?></a>
		</div>
<?php } ?>
	</div>
</div>
