...
<?php } ?>
			<div class="form-group row mb-3">
				<label for="post_title" class="col-sm-2 control-label">제목 (국문)</label>
...
<?php } ?>
			...
			<div class="form-group row mb-3">
				<label for="post_title" class="col-sm-2 control-label">제목 (영문)</label>
				<div class="col-sm-10" style="display:table;">
					<input type="text" class="form-control" name="post_title_en" id="post_title_en" value="<?php echo set_value('post_title_en', element('post_title_en', element('post', $view))); ?>" />
				</div>
			</div>
...
<?php } ?>
			<div class="form-group row mb-3">
				<div class="col-sm-12">
					<span class="d-inline-flex mb-3 px-2 py-1 fw-semibold text-success-emphasis bg-success-subtle border border-success-subtle rounded-2 w-100">내용 (국문)</span>
...
<?php } ?>
					...
					<span class="d-inline-flex mb-3 px-2 py-1 fw-semibold text-success-emphasis bg-success-subtle border border-success-subtle rounded-2 w-100 mt-5">내용 (영문)</span>
					<?php echo display_dhtml_editor('post_content_en', set_value('post_content_en', element('post_content_en', element('post', $view))), $classname = 'form-control dhtmleditor', $is_dhtml_editor = element('use_dhtml', element('board', $view)), $editor_type = $this->cbconfig->item('post_editor_type')); ?>
...
<?php } ?>
			<div class="border_button text-center mt20">
				<button type="button" class="btn btn-secondary btn-history-back">취소</button>
...
check_byte('post_content_en', 'char_count');
$(function() {
	$('#post_content').on('keyup', function() {
		check_byte('post_content', 'char_count');
	});
	$('#post_content_en').on('keyup', function() {
		check_byte('post_content_en', 'char_count');
	});
});
<?php } ?>
...
$(function() {
	$('#fwrite').validate({
		rules: {
			...
			post_content_en : { required_smarteditor :true }, 
<?php if (element('is_post_name', element('post', $view))) { ?>
