...
(function(App){
	...
	.on('coupon:tap click.coupon','.coupon-square', function(){
		...
		if(!couponNo){
			...
			$('#modalPrice').val(''); $('#modalCode').val(''); 

			// Kakao Link Reset
			$('.divCouponPriceSet').show();
			$('#btnModalUse').show();
			$('.divCouponLinkSet').empty().hide();
		...
		$.getJSON('/api/coupon/detail/'+couponNo,function(c){
			...
			const titleDisplay = (c.is_use === 'W' ? '<span class="badge bg-danger me-2">[임시 저장]</span>' : '') + title;
			$('#modalCouponTitle').html(titleDisplay);
			...
			if (c.kakao_link) {
				$('.divCouponPriceSet').hide();
				$('#btnModalUse').hide();
				var linkText = App.IS_DOMESTIC ? '카카오 쿠폰 링크' : 'Kakao Coupon Link';
				$('.divCouponLinkSet').html('<a href="'+c.kakao_link+'" target="_blank" rel="noopener" class="btn btn-warning btn-lg w-100 fw-bold p-3" style="color:#000;">'+linkText+'</a>').show();
			} else {
				$('.divCouponPriceSet').show();
				$('#btnModalUse').show();
				$('.divCouponLinkSet').empty().hide();
	...
	$(document).off('click.couponUse','#btnModalUse').on('click.couponUse','#btnModalUse',function(e){
		...
		.done(function(resp){
			...
			if (resp.kakao_link) { 
				var linkText = App.IS_DOMESTIC ? '카카오 쿠폰 링크' : 'Kakao Coupon Link';
				$('#resultLine2').html('<a href="'+resp.kakao_link+'" target="_blank" rel="noopener" class="btn btn-lg fw-bold btn-warning w-100 p-3" style="color:#000;">'+linkText+'</a>');
