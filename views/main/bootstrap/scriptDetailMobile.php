					...
					if(!arr.length){
						...
						groups.forEach((group,gi)=>{
							const cards = group.map(c=>{
								...
								var linkBtn = c.kakao_link ? `<div class="mt-1"><a href="${c.kakao_link}" target="_blank" rel="noopener" class="btn btn-warning w-100" style="font-size:10px; padding:2px 0; color:#000; border:none; font-weight:bold;">${App.IS_DOMESTIC?'카카오 쿠폰':'Kakao Coupon'}</a></div>` : '';

								const titleDisplay = (c.is_use === 'W' ? '<span class="text-danger small fw-bold me-1">[임시 저장]</span>' : '') + title;
								return `<div><div class="coupon-square" style="background-image:url('${thumb}')" role="button" data-coupon="${c.coupon_no}" data-store="${no}" title="${title}"></div><div class="coupon-meta"><div class="coupon-title">${titleDisplay}</div><div class="coupon-sub">${dcType} ${dcTxt}</div>${priceHtml}${linkBtn}</div></div>`;
							}).join('');
