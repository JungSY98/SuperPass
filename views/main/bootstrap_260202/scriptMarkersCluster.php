...
(function(App){
	...
	/* === Cluster helpers (Google / Kakao) =================================== */
	App.__cluster = App.__cluster || null;

	/** 기존 클러스터 제거 */
	App.resetCluster = function(){
		try {
		if (!App.__cluster) return;
		if (App.MAP?.provider==='kakao' && App.__cluster.setMap) {
			App.__cluster.clear();							 // 마커 내부풀 제거
			App.__cluster.setMap(null);					// 지도 연결 해제
		} else if (App.MAP?.provider==='google') {
			// @googlemaps/markerclusterer: setMap(null) 대신 clearMarkers()
			if (App.__cluster.clearMarkers) App.__cluster.clearMarkers();
			App.__cluster = null;
		}
		} catch(_) {}
		App.__cluster = null;
	...
	/** Google MarkerClusterer 라이브러리 보장 로더 */
	App.ensureGoogleClusterLib = function(cb){
		if (window.markerClusterer?.MarkerClusterer || window.MarkerClusterer) { cb && cb(); return; }
		// scriptMapInit.php 또는 전역 util에 loadScript가 있으면 사용
		const src = 'https://unpkg.com/@googlemaps/markerclusterer/dist/index.min.js';
		const exists = document.querySelector(`script[src="${src}"]`);
		if (exists) { exists.addEventListener('load', ()=>cb&&cb()); return; }
		const s=document.createElement('script'); s.src=src; s.async=true; s.defer=true;
		s.onload = ()=> cb && cb();
		s.onerror= ()=> cb && cb(); // 실패해도 앱이 죽지 않게
	...
	function setView(map, lat, lng, zoomOrLevel){
		if (App.MAP.provider === 'kakao' && window.kakao) {
			...
			animateZoom(map, zoomOrLevel);
			map.setCenter(new google.maps.LatLng(lat, lng));
		}
	}
	function animateZoom(map, delta) {
		const currentZoom = map.getZoom();
		let newZoom = currentZoom + delta;
		let duration = 5000; // 애니메이션 지속 시간 (ms)
		let startTime = null;
		...
		function step(currentTime) {
			...
			const easedProgress = progress; // 필요에 따라 easing 함수 적용
			map.setZoom(currentZoom + Math.round(delta * easedProgress));
	...
	function dimOtherMarkers(except){
		...
		App.__markers.forEach(m => {
			setMarkerOpacity(m, (m === except) ? 1 : 0.4)
		});
	...
	App.handleMarkerClick = function(id, marker){
		...
		$.post('/api/store_click', { store_no: id }).always(function(){});
		...
		if (App.MAP.provider === 'kakao'){
			...
			setTimeout(()=> {
				...
				setView(map, lat, lng, 2);
				// 락 해제는 상세 오프캔버스 열린 뒤/닫힘에서 수행
			}, 400);
		} else {
			const focusZm = App.FOCUS_ZOOM?.google ?? 25;
			const maxZm	 = App.MAP_LOCK?.google?.maxZoom ?? 25;
			map.panTo(new google.maps.LatLng(lat, lng));
			setTimeout(()=> {
				// 같은 마커 재클릭이면 현재 줌 유지 + 센터만 보정
				if (sameTarget) {
					setView(map, lat, lng, 25);
				} else {
	...
	App.renderMarkers = async function(rows){
		...
		for (const r of rows){
			...
			map:null,														 // ★ 클러스터러가 map에 올립니다
			position:pos, image:ic.kakao,
		...
		// 2) Kakao MarkerClusterer 생성/적용
		App.__cluster = new kakao.maps.MarkerClusterer({
			map: App.MAP.map,
			averageCenter: true,
			minLevel: 3,						// 이 레벨보다 작게(=더 멀리) 보면 클러스터링
			gridSize: 60,					 // 클러스터 그리드(px) — 필요에 맞게 조정
			disableClickZoom: false // 클러스터 클릭 시 자동 확대 허용
		});
		App.__cluster.addMarkers(App.__markers);
		...
		// 2) Google MarkerClusterer 생성/적용
		App.ensureGoogleClusterLib(function(){
			// UMD: window.markerClusterer.MarkerClusterer 또는 구버전 window.MarkerClusterer
			const MCNS = window.markerClusterer;
			if (MCNS && MCNS.MarkerClusterer){
			App.__cluster = new MCNS.MarkerClusterer({ map: App.MAP.map, markers: App.__markers });
			} else if (window.MarkerClusterer) {
			// (구버전) markerclustererplus 호환
			App.__cluster = new window.MarkerClusterer(App.MAP.map, App.__markers, {
				gridSize: 60, maxZoom: 20
			});
			} else {
			// 라이브러리 로드 실패 시에도 지도는 동작해야 함: 마커를 직접 올림 (비클러스터)
			App.__markers.forEach(m=> m.setMap(App.MAP.map));
			}
		});
		}
	...
	App.bindListFocus = function(itemSelector='.place-item', idAttr='data-no'){
		document.addEventListener('click', function(e){
			...
			if (mark){
				...
				if (row && (row.mapLat && row.mapLng)) App.focusStore({ lat:+row.mapLat, lng:+row.mapLng, plus:3 });
			}
		});
	};
