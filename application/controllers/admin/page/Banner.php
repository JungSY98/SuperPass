...
class Banner extends CB_Controller
{
	...
	public function index()
	{
		...
		if (empty($is_admin)) {
			...
			$storeNo = element("storeNo", element(0, $storeD));
			$where['ban_store_id'] = $storeNo;
		}
	...
	public function write($pid = 0)
	{
		...
		if ($form_validation === false OR $file_error !== '') {
			...
			$updatedata = array(
				...
				'menuColor' => $this->input->post('menuColor', true),
			);
			if (empty($is_admin)) {
				$storeD = $this->site->selectCI("storeNo", "store_member", array("memNo"=>$this->member->item("mem_id")), "");
				$storeNo = element("storeNo", element(0, $storeD));
				$updatedata['ban_store_id'] = $storeNo;
			}
