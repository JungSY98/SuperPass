...
class Match extends CB_Controller
{
	...
	public function index() {
		...
		$view['rList'] = $this->site->selectQuery("select mem_id, mem_userid, mem_username, (select count(no) as matchCnt from mf_apply as A where A.researcherID = M.mem_userid  and `times` = '".$where['times']."'  and A.category = '".$view['viewCate']."') as matchCateCnt, (select count(no) as matchCnt from mf_apply as A where A.researcherID = M.mem_userid ) as matchTotalCnt from mf_member as M where mem_userid like 'researcher%' order by mem_userid asc");
