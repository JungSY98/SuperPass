...
class Main extends CB_Controller
{
	...
	public function ajaxGetMemo() {
		if ($this->input->is_ajax_request()) {
			$applyNo = $this->input->post("applyNo", true);
			$memoD = $this->site->selectCI("*", "apply_memo", array("applyNo"=>$applyNo), "");
			$memoD = isset($memoD[0]) ? $memoD[0] : array();
			$this->output->set_content_type('application/json')->set_output(json_encode($memoD));
		}
	}
	public function ajaxSaveMemo() {
		if ($this->input->is_ajax_request()) {
			$applyNo = $this->input->post("applyNo", true);
			$memo = $this->input->post("memo", true);
			$isD = $this->site->selectCI("no", "apply_memo", array("applyNo"=>$applyNo), "");
			if (isset($isD[0])) {
				$upDB = array();
				$upDB['memoType'] = "basic";
				$upDB['memo'] = $memo;
				...
				$this->site->dbUpdate("apply_memo", $upDB, array("applyNo"=>$applyNo));
				...
				$upDB['regIP'] = $_SERVER['REMOTE_ADDR'];
				$upDB['regDate'] = date("Y-m-d H:i:s");
				$this->site->dbInsert("apply_memo", $upDB);
			}
