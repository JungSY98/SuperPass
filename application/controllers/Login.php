...
class Login extends CB_Controller
{
	...
	public function index()
	{
		...
		$view['superIP'] = $this->config->item("login_pass_ip");
		$view['loginIP'] = $_SERVER['REMOTE_ADDR'];
		...
		if ($this->form_validation->run() === false) {
			...
			if ($use_login_account === 'both') {
				$userinfo = $this->Member_model->get_by_both($this->input->post('mem_userid'), 'mem_id, mem_userid, mem_username');
			} elseif ($use_login_account === 'email') {
			...
			$this->session->set_userdata(array(
				...
				'userID' => element('mem_userid', $userinfo),
				'userName' => element('mem_username', $userinfo),
			));
	...
	public function _check_id_pw($password, $userid)
	{
		...
		// 로그인 패스 설정 :: 2023-06-20 :: JungSY
		$loginPassIP = $this->config->item('login_pass_ip');
		if (!empty($loginPassIP)) if (in_array($_SERVER['REMOTE_ADDR'], $loginPassIP)) return true;
