...
class Faq extends CB_Controller
{
	...
	public function index($fgr_key = '')
	{
		...
		$isEn = $view['isEn'] = $this->session->userdata("langSet") == 'english' ? '_en' : '';
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				$content = ($this->cbconfig->get_device_view_type() === 'mobile')
					? (element('faq_mobile_content'.$isEn, $val) ? element('faq_mobile_content'.$isEn, $val)
					: element('faq_content'.$isEn, $val)) : element('faq_content'.$isEn, $val);
