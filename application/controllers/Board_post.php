...
class Board_post extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
	protected $models = array('Post', 'Post_meta', 'Post_extra_vars', 'Post_link');
	...
	function __construct()
	{
		...
		$this->langSet = $this->session->userdata("langSet") == 'english' ? '_en' : '';
	...
	public function post($post_id = 0, $print = false)
	{
		...
		$view['view']['post']['title'] = $view['view']['post']['post_title'.$this->langSet];
			...
			if ($is_blind === false OR $is_admin !== false
				...
				$view['view']['post']['content'] .= $file_player . $link_player
					. display_html_content(
						element('post_content'.$this->langSet, $post),
	...
	public function _get_list($brd_key, $from_view = '')
	{
			...
			foreach ($noticeresult as $key => $val) {
				...
				if ($this->cbconfig->get_device_view_type() === 'mobile') {
					$noticeresult[$key]['title'] = element('mobile_subject_length', $board)
						? cut_str(element('post_title'.$this->langSet, $val), element('mobile_subject_length', $board))
						: element('post_title'.$this->langSet, $val);
				} else {
					$noticeresult[$key]['title'] = element('subject_length', $board)
						? cut_str(element('post_title'.$this->langSet, $val), element('subject_length', $board))
						: element('post_title'.$this->langSet, $val);
				}
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				...
				$result['list'][$key]['post_link'] = $this->Post_link_model->get('', '', array('post_id' => element('post_id', $val)), '', '', 'pln_id', 'ASC');
