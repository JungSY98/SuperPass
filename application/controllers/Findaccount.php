	...
	public function index()
	{
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->input->post('findtype') === 'findidpw') {
				...
				$langField = element("langSet", $_SESSION)=='english' ? '_en' : '';

				$title = str_replace(
					$searchconfig,
					$replaceconfig,
					$this->cbconfig->item('send_email_findaccount_user_title'.$langField)
				);
				$content = str_replace(
					$searchconfig,
					$replaceconfig_escape,
					$this->cbconfig->item('send_email_findaccount_user_content'.$langField)
				);
