...
class Managelayout extends CI_Controller
{
	...
	function admin($config = array(), $device_view_type = '')
	{
		...
		if ($CI->member->is_admin() != 'super') {
			$admin_auth = $CI->site->selectCI("*", "admin_auth", array("mem_userid"=>$CI->member->item("mem_userid")), "");
			if (!empty($admin_auth)) foreach($admin_auth as $_k => $aD) {
				$data['admin_auth'][$aD['admin_role']] = true;
			}
		}
...
*/
	...
	function front($config = array(), $device_view_type = '')
	{
		...
		if ($CI->input->is_ajax_request() === false) {
			...
			$CI->lang->load("website", element("langSet", $_SESSION));
			$data['siteTitle'] = $CI->lang->language['site'];
