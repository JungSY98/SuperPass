...
function getAttachFile($objD, $hr=false, $isModify=false) {
	//$CI = & get_instance();
	if (element('attachFile', $objD)) {
		$html = '<div style="clear:both;">';
		$fileD = json_decode(element('attachFile', $objD), true);
		if ($hr && is_array($fileD)) $html .= '<hr style="clear:both;">'; // <p class="btn btn-dark">첨부파일</p>
		if (is_array($fileD)) {
			foreach($fileD as $k => $fD) {
				$html .= '<div class="input-group mb-1">';
				$html .= '<span class="input-group-text" style="font-size:13px;"><i class="fa fa-file-o"></i></span>';
				$html .= '<a href="/downFiles/'.$fD['fileName'].'/'.urlencode($fD['clientName']).'/" class="btn btn-light" style="border:1px solid #ddd;font-size:13px;">'.$fD['clientName'].'</a>';
				if ($isModify) $html .= '<button type="button" class="btn btn-danger" onclick="return fnDeleteFile(this, '."'".element('fileName', $fD)."'".')"><i class="fa fa-trash"></i></button>';
				$html .= '</div>';
			}
		}
		$html .= '</div>';
...
function getManagerName($userID) {
	...
	if ($userID=='manage') {
		...
		if (preg_match("/,/i", $userID)) {
			$targetID = explode(",", $userID);
			if (in_array(element("userID", $_SESSION), $targetID)) {
				$sql = "select GROUP_CONCAT(name SEPARATOR ',') as name from ds_cowork_manager where userID in ( '".element("userID", $_SESSION)."' ) ";
				$name = $CI->site->selectQuery($sql);
				$name = isset($name[0]) ? $name[0]['name'] : '';
				return $name." 외 ".(count($targetID)-1)."명";
			} else {
				return "총 ".count($targetID)."명";
			}
		} else {
			if (element("userID", $_SESSION)==$userID) {
				$sql = "select GROUP_CONCAT(name SEPARATOR ',') as name from ds_cowork_manager where userID in ( '".element("userID", $_SESSION)."' ) ";
				$name = $CI->site->selectQuery($sql);
				$name = isset($name[0]) ? $name[0]['name'] : '';
				return $name;
			} else {
				$sql = "select GROUP_CONCAT(name SEPARATOR ',') as name from ds_cowork_manager where userID in ( '".$userID."' ) ";
				$name = $CI->site->selectQuery($sql);
				$name = isset($name[0]) ? $name[0]['name'] : '';
				return $name;
			}
...
function echoPhoneNumber($number) {
	$number = trim($number);
	$no1 = $no2 = $no3 = '';
	if (preg_match("/-/i", $number)) {
		...
		if (strlen($number)==10) {
			if (substr($number, 0,2)=='02') {
				$no1 = substr($number, 0,2);
				$no2 = substr($number, 2,4);
				$no3 = substr($number, 5,4);
			} else {
				$no1 = substr($number, 0,3);
				$no2 = substr($number, 3,3);
				$no3 = substr($number, 5,4);
			}
		} else if (strlen($number)==11) {
			$no1 = substr($number, 0,3);
			$no2 = substr($number, 3,4);
			$no3 = substr($number, 7,4);
		} else if (strlen($number)==9) {
			$no1 = substr($number, 0,2);
			$no2 = substr($number, 2,3);
			$no3 = substr($number, 5,4);
		} else {
			return $number;
		}
